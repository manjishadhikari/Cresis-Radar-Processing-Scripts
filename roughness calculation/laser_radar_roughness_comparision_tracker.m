...
if strcmpi(param.sched.type,'custom_torque')
  ...
  torque_compile('laser_radar_roughness_comparision.m',ctrl.sched.hidden_depend_funs,ctrl.sched.force_compile);
...
for line_idx = 1:length(param.lines)
  task_param.proc_line= param.lines(line_idx);
    ...
    % Execute tasks/jobs
    fh = @laser_radar_roughness_comparision_task;
    ...
    if strcmp(param.sched.type,'custom_torque')
      ...
      ctrl = torque_create_task(ctrl,fh,1,arg,create_task_param);
       fprintf(' sub %d/%d:  Matlab job id: %d. Torque Job id: %d. Processing Line number %d (%s) \n',ctrl.batch_id,ctrl.job_id_list(line_idx),line_idx,ctrl.job_id_list(line_idx), task_param.proc_line,datestr(now,'HH:MM:SS'));
